...
namespace SalePCServiceImplementDataBase.Implementations
{
    public class ReportServiceDB : IReportService
    {
        ...
        public ReportServiceDB(AbstractPCDbContext context)
        {
        ...
        public void SavePCPrice(ReportBindingModel model)
        {
            ...
            try
            {
               ...
               missing);
                ...
                var PCs = context.PCs.ToList();
            //создаем таблицу
 var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, PCs.Count, 2, ref
               missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = PCs[i].PCName;
                    table.Cell(i + 1, 2).Range.Text = PCs[i].Price.ToString();
                }
        ...
        public List<StocksLoadViewModel> GetStocksLoad()
        {
            ...
            .ToList()
           .GroupJoin(
            context.StockHardwares
            .Include(r => r.Hardware)
           .ToList(),
            stock => stock,
           stockHardware => stockHardware.Stock,
           (stock, stockCompList) =>
            new StocksLoadViewModel
            {
                StockName = stock.StockName,
                TotalCount = stockCompList.Sum(r => r.Count),
                Hardwares = stockCompList.Select(r => new Tuple<string,
    int>(r.Hardware.HardwareName, r.Count))
            })
            .ToList();
        }
        ...
        public void SaveClientOrders(ReportBindingModel model)
        {
           ...
           iTextSharp.text.Font.BOLD);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].PCName, fontForCells));
                table.AddCell(cell);
